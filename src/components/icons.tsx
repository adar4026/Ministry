...
// TASK_066 — Home header hamburger + drawer close, same 24x24 stroke style.
// A real SVG glyph, deliberately not the "☰" emoji/text character: it
// scales and recolors like every other icon here and renders identically
// on web and native.
...
export function XIcon({ size = 22, color }: IconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
// TASK_073 — «Настройки» row in the drawer: a plain outline gear.
...
// TASK_073 — «✓ Отметить служение» / a marked day.
...
// TASK_078 — Finance's `.dh-theme` sun / moon (the same 24-px paths).
...
export function MoonIcon({ size = 22, color }: IconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z" stroke={color} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" />
    </Svg>
  );
}
