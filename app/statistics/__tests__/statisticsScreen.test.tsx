...
describe("Статистика — год", () => {
  it("opens on the current year with the header, hero total, average, tiles, months and the all-time link", async () => {
    const r = await render([
      session("2026-01-10", 34 * 60 + 20),
      session("2026-03-01", 52 * 60),
      session("2026-03-15", 30),
      session("2026-09-02", 60),
    ]);
    const t = texts(r);
    expect(t).toContain("Статистика");
    expect(r.root.findByProps({ testID: "stats-year" }).props.children).toBe(2026);
    expect(t).toContain("Текущий год");
    // Hero: 34:20 + 52:30 + 1:00 = 87 ч 50 м, average over the 3 months with data.
    const hero = r.root.findByProps({ testID: "stats-hero" });
    expect(hero.props.accessibilityLabel).toContain("87 ч 50 м");
    expect(t).toContain("Служение за год");
    expect(t).toContain("Среднее 29 ч 17 м / месяц");
    expect(t).toContain("Кратко");
    expect(t).toContain("Динамика");
    expect(t).toContain("По месяцам");
    expect(t).toContain("За всё время");
    // Twelve month rows; March is the busiest and tappable, June is empty.
    for (const m of ["Январь", "Февраль", "Март", "Июнь", "Декабрь"]) expect(t).toContain(m);
    ...
    // Tiles: 4 service days, busiest March, 3 of 12 months.
    expect(t).toContain("Дней служения");
    expect(t).toContain("4");
    expect(t).toContain("3 из 12");
    // No previous-year data → no comparison block.
    expect(r.root.findAllByProps({ testID: "stats-comparison" })).toHaveLength(0);
    expect(t).not.toContain("Сравнение");
  });

  it("tapping a month with data opens /statistics/month/YYYY-MM; an empty month is not a button", async () => {
    const r = await render([session("2026-03-01", 60)]);
  ...
  it("the year stepper moves between the earliest year with data and the current year, never into the future", async () => {
    ...
    // 2025 has no data → empty state, still on the stepper.
  ...
  it("shows the comparison block only when the previous year has data, with neutral figures", async () => {
  ...
  it("a legacy-only year shows the total, marks the months as monthly totals and does not invent days", async () => {
    mockParams = { year: "2024" };
    const r = await render([], [record(2024, 5, 37.5), record(2024, 6, 40)]);
    const t = texts(r);
    expect(r.root.findByProps({ testID: "stats-year" }).props.children).toBe(2024);
...
describe("Статистика — за всё время", () => {
  ...
  it("switches via the segment: lifetime hero, facts, years newest first", async () => {
    ...
    expect(labels).toEqual(["stats-year-2026", "stats-year-2024", "stats-year-2023"]);
  ...
  it("tapping a year in «По годам» returns to that year's statistics", async () => {
    const r = await render(data, legacy);
    await press(r, "stats-mode-all");
    await press(r, "stats-year-2024");
    expect(r.root.findByProps({ testID: "stats-year" }).props.children).toBe(2024);
    expect(r.root.findByProps({ testID: "stats-hero" }).props.accessibilityLabel).toContain("10 ч 30 м");
  });
