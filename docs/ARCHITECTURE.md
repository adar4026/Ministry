...
│   │   ├── serviceStats.ts     # Индекс + чистые расчёты «Статистики» по служебным годам (TASK_081/082)
...
Период раздела — **служебный год** (TASK_082): те же `serviceYearEndYear`
/ `serviceYearMonths` / `currentServiceYearEndYear` / `serviceYearLabel`
из `src/data/serviceYear.ts`, что и у `/hours/stats` и History — год
идентифицируется годом окончания и подписан «2025–2026» (= Сентябрь 2025
— август 2026); в сентябре 2026 текущий год — 2026–2027. Месяцы идут
Сен → Авг, «По годам» и «Сравнение» — тоже по служебным годам; единица
хранения и детализации — календарный месяц (`/statistics/month/2025-09`).
Среднее в месяц = итог / число месяцев с записями (будущие/пустые месяцы
не искажают). Легаси-месяцы (`HourRecord`) дают итог, но не дни — UI пишет
