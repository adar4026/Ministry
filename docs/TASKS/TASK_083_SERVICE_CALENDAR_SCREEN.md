...
(`63fa025`), запушено в `origin/main`, задеплоено на GitHub Pages
(`gh-pages` `720f924`), проверено на production (§5б).
...
Закоммичено `63fa025` (`main`, было `7c79eeb`; 19 файлов), запушено.
Задеплоено: `gh-pages` `720f924` (было `9eb2bdf`), бандл
`entry-8b2c8ee29d58084e16718532a59437fc.js` — хэш идентичен локальному
`npx expo export` и ответу `adar4026.github.io` (вторая попытка опроса
после ~20 с CDN-задержки); изолированная вкладка загрузила именно этот
`entry-*.js`. Проверено на production (390 px): `/Ministry/calendar`
открывается — «Календарь служения», Месяц/Год/Всё время, «Сентябрь 2026 ·
Текущий месяц», сетка в Ministry-скине, `[role=tab]` = 0 (tab bar не
смонтирован), `scrollWidth === 390`; на этом origin записей нет → «Нет
записей за этот месяц»; консоль пуста. Полная проверка с данными — на
dev-сервере (§5а), бандл тот же по хэшу. Ожидает проверки владельца на
iPhone с реальными записями.
